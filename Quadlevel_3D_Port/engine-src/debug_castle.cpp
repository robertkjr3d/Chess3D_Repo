...
static void generate_and_display_mate2(int attempts = 500, int time_limit_ms = 5000) {
    std::printf("=== Generating Mate-in-2 Puzzle ===\n");
    std::printf("Attempts: %d, Time limit per verification: %dms\n\n", attempts, time_limit_ms);

    MateInTwoGenerator generator(50);
    
    auto start = std::chrono::steady_clock::now();
    MateInTwoPuzzle puzzle = generator.generate_puzzle(attempts, time_limit_ms);
    auto end = std::chrono::steady_clock::now();
    int elapsed_ms = int(std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count());

    if (puzzle.fen.empty()) {
        std::printf("ERROR: Failed to generate mate-in-2 puzzle after %d attempts (%dms)\n",
                    attempts, elapsed_ms);
        return;
    }

    std::printf("✓ Puzzle generated in %dms\n\n", elapsed_ms);
    std::printf("FEN: %s\n", puzzle.fen.c_str());
    std::printf("Side to move: %s\n", puzzle.side_to_move == WHITE ? "White" : "Black");
    std::printf("Solution (first move): %s\n\n", puzzle.first_move.c_str());

    // Load and display the position
    Position3D pos;
    if (!pos.set_fen(puzzle.fen)) {
        std::fprintf(stderr, "ERROR: Failed to load puzzle FEN\n");
        return;
    }
    ...
    std::printf("\n╔════════════════════════════════════════╗\n");
    std::printf("║  PUZZLE: Find mate in 2 moves!        ║\n");
    std::printf("║  Solution: Play %s                     ║\n", puzzle.first_move.c_str());
    std::printf("╚════════════════════════════════════════╝\n");
}
...
int main(int argc, char* argv[]) {
    ...
    // Check for mate2 command first (new feature)
    if (argc >= 2 && std::string(argv[1]) == "mate2") {
        int attempts = 120;        // default (kept modest for CLI responsiveness)
        int time_limit_ms = 400;   // default per verification

        // Parse optional parameters: mate2 <attempts> <time_ms>
        if (argc >= 3) {
            attempts = std::atoi(argv[2]);
            if (attempts < 1) attempts = 1;
            if (attempts > 10000) attempts = 10000;
        }
        if (argc >= 4) {
            time_limit_ms = std::atoi(argv[3]);
            if (time_limit_ms < 50) time_limit_ms = 50;
            if (time_limit_ms > 30000) time_limit_ms = 30000;
        }

        generate_and_display_mate2(attempts, time_limit_ms);
        return 0;
    }

    // Check for interactive mode (existing feature)
