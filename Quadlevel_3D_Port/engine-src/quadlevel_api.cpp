...
struct QLContext {
    ...
    MateInTwoGenerator* puzzle_gen = nullptr;
};
...
QUADLEVEL_API const char* ql_version(void) {
...
QUADLEVEL_API void* ql_create(void) {
    ...
    c->pos.set_startpos();
    c->puzzle_gen = new MateInTwoGenerator(100);  // Cache up to 100 puzzles
...
QUADLEVEL_API int ql_make_move(void* ctx, const char* move_str) {
    if (!move_str)
        return 0;
...
// ─────────────────────────────────────────────
//  Mate-in-2 Puzzle Generation
// ─────────────────────────────────────────────

QUADLEVEL_API const char* ql_generate_mate2_puzzle(void* ctx) {
    auto* c = ctx_of(ctx);
    if (!c->puzzle_gen)
        return to_c_str("");
    
    MateInTwoPuzzle puzzle = c->puzzle_gen->generate_puzzle(500, 3000);
    
    if (puzzle.fen.empty())
        return to_c_str("");
    
    // Format: "FEN|first_move"
    std::string result = puzzle.fen + "|" + puzzle.first_move;
    return to_c_str(result);
}

QUADLEVEL_API const char* ql_get_cached_mate2_puzzle(void* ctx) {
    auto* c = ctx_of(ctx);
    if (!c->puzzle_gen || c->puzzle_gen->cache_size() == 0)
        return to_c_str("");

    std::uniform_int_distribution<size_t> dist(0, c->puzzle_gen->cache_size() - 1);
    ...
    const MateInTwoPuzzle* puzzle = c->puzzle_gen->get_cached_puzzle(idx);
    if (!puzzle)
        return to_c_str("");
...
QUADLEVEL_API int ql_is_mate_in_2(void* ctx) {
    auto* c = ctx_of(ctx);
    if (!c->puzzle_gen)
        return 0;
    
    return c->puzzle_gen->is_mate_in_two(c->pos, 3000) ? 1 : 0;
}
