    ...
    export default function App() {
      ...
      const evaluatePosition = useCallback((pieces, color) => {
        ...
        for (const p of pieces) {
          ...
          score += side * v * 40 * centrality(p.x, p.y, p.z);
        ...
        return moves.slice().sort((a, b) => {
          ...
          try {
            ...
            try {
              ...
              const earlyQueenPenalty = (ply < 20) ? 800 : 0; // penalize queen moves in first ~10 moves
              const moverA = (pieces || []).find(pp => pp.id === a.moverId);
              const moverB = (pieces || []).find(pp => pp.id === b.moverId);
              if (moverA && moverA.t === 'Q') scoreA -= earlyQueenPenalty;
              if (moverB && moverB.t === 'Q') scoreB -= earlyQueenPenalty;
              const earlyRookPenalty = (ply < 20) ? 600 : 0; // discourage early rook moves unless capturing
              if (moverA && moverA.t === 'R' && capA === 0) scoreA -= earlyRookPenalty;
              if (moverB && moverB.t === 'R' && capB === 0) scoreB -= earlyRookPenalty;
            } catch (e) {}
            ...
            try {
              if (typeof lastMove !== 'undefined' && lastMove && lastMove.to) {
                ...
                if (a.x === lx && a.y === ly && a.z === lz) scoreA += 5000;
                if (b.x === lx && b.y === ly && b.z === lz) scoreB += 5000;
              }
            } catch (e) {}
      ...
      }, [simulateMove, attackersOfSquare, moveHistory, lastMove]);
      ...
      useEffect(() => {
        ...
        const t = setTimeout(() => { (async () => {
          try {
            const moves = getAllLegalMoves(piecesState, aiSide || 'black');
            if (!moves || moves.length === 0) return;
            
            // iterative deepening with time cap and move ordering
            const maxMillis = 800; // time cap in ms
            ...
            // strict tactical filter: remove any candidate move that allows the opponent
            // to perform an immediate capture with positive SEE on any AI piece or on the moved piece.
            try {
              ...
              for (const m of orderedMoves) {
                try {
                  ...
                  for (const oc of oppMovesTmp) {
                    ...
                    try {
                      const seeVal = staticExchangeEval(nextTmp, oc.x, oc.y, oc.z, opp);
                      if (seeVal > 0) { bad = true; break; }
                    } catch (ee) {}
                  }
                  if (!bad) safe.push(m);
                } catch (ee) {}
              }
              if (safe.length > 0) orderedMoves = safe;
            } catch (e) { /* keep original ordering on error */ }
            // try depths 1..maxDepth with early stop on time
            for (let depth = 1; depth <= maxDepth; depth++) {
              ...
              for (const m of orderedMoves) {
                ...
                try {
                  // penalize moves that allow opponent an immediate capture of the moved piece if it's high-value
                  const moved = next.find(p => p.id === m.moverId) || null;
                  if (moved) {
                    // check opponent captures of the moved piece
                    const oppMoves2 = getAllLegalMoves(next, opp) || [];
                    const canCaptureMoved = oppMoves2.some(oc => oc.x === moved.x && oc.y === moved.y && oc.z === moved.z);
                    if (canCaptureMoved) {
                      const vals = { p: 1, N: 3, B: 3, R: 5, Q: 9 };
                      const v = vals[moved.t] || 0;
                      if (v >= 3) {
                        // use static exchange evaluation to determine true material swing
                        try {
                          const see = staticExchangeEval(next, moved.x, moved.y, moved.z, opp);
                          if (see > 0) finalScore -= see * 1200;
                          else finalScore -= v * 600;
                        } catch (ee) { finalScore -= v * 1000; }
                      }
                    }
                    // additionally, check for opponent immediate captures of ANY high-value AI piece after this candidate move
                    try {
                      const oppAll = oppMoves2;
                      for (const oc of oppAll) {
                        const targetOcc = next.find(pp => pp.x === oc.x && pp.y === oc.y && pp.z === oc.z && pp.color === aiSide);
                        if (!targetOcc) continue;
                        const vals = { p: 1, N: 3, B: 3, R: 5, Q: 9 };
                        const tv = vals[targetOcc.t] || 0;
                        if (tv < 3) continue;
                        try {
                          const see2 = staticExchangeEval(next, oc.x, oc.y, oc.z, opp);
                          if (see2 > 0) {
                            finalScore -= see2 * 1400; // stronger penalty for any positive SEE
                          }
                        } catch (eeee) {}
                      }
                    } catch (ee) {}
              ...
              // reorder moves by the latest best to improve next iteration
              if (best) orderedMoves = [best].concat(orderedMoves.filter(m => m !== best));
            }
            // blunder-avoidance: avoid moves that immediately hang material if alternative exists
            if (best) {
              try {
                const next = simulateMove(piecesState, best.moverId, { x: best.x, y: best.y, z: best.z });
                // if the moved piece can be captured immediately and material drops, penalize and try next candidate
                const opponent = aiSide === 'white' ? 'black' : 'white';
                const oppCaps = getAllLegalMoves(next, opponent) || [];
                const movedPieceAfter = next.find(p => p.id === best.moverId);
                let hangs = false;
                if (movedPieceAfter) {
                  for (const oc of oppCaps) {
                    if (oc.x === movedPieceAfter.x && oc.y === movedPieceAfter.y && oc.z === movedPieceAfter.z) { hangs = true; break; }
                  }
                }
                if (hangs) {
                  // try to pick another non-hanging move from orderedMoves
                  const alt = orderedMoves.find(m => m !== best && !(() => {
                    const n = simulateMove(piecesState, m.moverId, { x: m.x, y: m.y, z: m.z });
                    const caps = getAllLegalMoves(n, opponent) || [];
                    const moved = n.find(p => p.id === m.moverId);
                    if (!moved) return false;
                    for (const c of caps) if (c.x === moved.x && c.y === moved.y && c.z === moved.z) return true;
                    return false;
                  })()) || null;
                      if (alt) { try { console.debug('AI avoided hanging best move, switching to alt', { best, alt }); } catch (e) {} best = alt; }
                }
              } catch (e) {}
            }
            if (!best) best = moves[Math.floor(Math.random() * moves.length)];
            try { console.log('AI selected move (minimax)', { best, score: bestScore }); } catch (e) {}
            if (best) {
              try {
                // compute nothing here; applyMove will compute notation from stable snapshot
                const moverObj = piecesState.find(p => p.id === best.moverId) || null;
                try { console.debug('AI will applyMove', { best, moverObj, moveLock: moveLockRef.current }); } catch (e) {}
                // additional blunder check: if move hangs and loses material, try an alternative
                try {
                  const next = simulateMove(piecesState, best.moverId, { x: best.x, y: best.y, z: best.z });
                  const opponent = aiSide === 'white' ? 'black' : 'white';
                  const oppCaps = getAllLegalMoves(next, opponent) || [];
                  ...
                  if (hangs) {
                    // compute simple material before/after
                    const vals = { p: 1, N: 3, B: 3, R: 5, Q: 9 };
                    const sumMaterial = (arr, side) => arr.filter(q => q.color === side).reduce((s, q) => s + (vals[q.t] || 0), 0);
                    const beforeMat = sumMaterial(piecesState, aiSide) - sumMaterial(piecesState, opponent);
                    const afterMat = sumMaterial(next, aiSide) - sumMaterial(next, opponent);
                    // if material drop is significant (>=1 pawn), try alternative
                    if ((beforeMat - afterMat) >= 1) {
                      const alt = orderedMoves.find(m => m !== best && !(() => {
                        const n = simulateMove(piecesState, m.moverId, { x: m.x, y: m.y, z: m.z });
                        const caps = getAllLegalMoves(n, opponent) || [];
                        const moved = n.find(p => p.id === m.moverId);
                  ...
                  // stronger safety check: compute worst-case SEE loss for best and alternatives
                  try {
                    const opponent2 = aiSide === 'white' ? 'black' : 'white';
                    const worstSeeFor = (m) => {
                      try {
                        const n = simulateMove(piecesState, m.moverId, { x: m.x, y: m.y, z: m.z });
                        const oppMoves = getAllLegalMoves(n, opponent2) || [];
                        let worst = 0;
                        for (const oc of oppMoves) {
                          const targetOcc = n.find(pp => pp.x === oc.x && pp.y === oc.y && pp.z === oc.z && pp.color === aiSide);
                          if (!targetOcc) continue;
                          try {
                            const s = staticExchangeEval(n, oc.x, oc.y, oc.z, opponent2);
                            if (s > worst) worst = s;
                          } catch (ee) {}
                        }
                        return worst;
                    ...
                    const bestWorst = worstSeeFor(best);
                    if (bestWorst > 0) {
                      // try to find zero-worst alternative
                      const safeAlt = orderedMoves.find(m => m !== best && worstSeeFor(m) === 0) || null;
                      if (safeAlt) { try { console.debug('AI switched to safe alternative avoiding SEE loss', { from: best, to: safeAlt }); } catch (e) {} best = safeAlt; }
                      else {
                        // pick move with minimal worst SEE
                        let minM = best; let minVal = bestWorst;
                        for (const m of orderedMoves) {
                          try {
                            const w = worstSeeFor(m);
                            if (w < minVal) { minVal = w; minM = m; }
                          } catch (eee) {}
                        }
                        if (minM !== best) { try { console.debug('AI chose minimal-SEE-loss alternative', { from: best, to: minM, fromWorst: bestWorst, toWorst: minVal }); } catch (e) {} best = minM; }
                      }
                    }
                  } catch (ee) {}
                } catch (e) {}
                try {
                  ...
                  try { console.debug('AI applyMove() called', { moverId: best.moverId }); } catch (e) {}
                } catch (err) { try { console.debug('AI applyMove() threw', err); } catch (e) {} }
              } catch (e) { console.debug('failed to apply AI move', e); }
            }
          } catch (e) { console.debug('AI move failed', e); }
        })(); }, thinkDelay);
